...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  ...
  const b = (y2 - y1) ** 2;
  const d = a + b;
  return Math.sqrt(Math.sign(d) * d);
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const aLenght = Math.sqrt(x1 ** 2 + y1 ** 2);
  const bLenght = Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(ab / (aLenght * bLenght));
}
